	...
	class YITH_WCMC_Admin {
		...
		public function __construct() {
			$this->available_tabs = apply_filters(
				'yith_wcmc_available_admin_tabs',
				array(
			...
			add_action(
				'woocommerce_admin_field_yith_wcmc_integration_status',
				array(
					$this,
			...
			add_filter(
				'plugin_action_links_' . plugin_basename( YITH_WCMC_DIR . 'init.php' ),
				array(
					$this,
					'action_links',
				)
		...
		public function enqueue() {
			...
			if ( $screen && in_array( $screen->id, array( 'shop_order', 'yith-plugins_page_yith_wcmc_panel' ), true ) ) {
				wp_enqueue_style( 'yith-wcmc-admin', YITH_WCMC_URL . 'assets/css/admin/yith-wcmc.css', array(), YITH_WCMC_VERSION );
				wp_enqueue_script(
					'yith-wcmc-admin',
					YITH_WCMC_URL . 'assets/js/admin' . $path . '/yith-wcmc' . $prefix . '.js',
					array(
						'jquery',
						'jquery-blockui',
					),
...
This will delete all store data from your Mailchimp account, including Products, Coupons, Orders and Customers",
		...
		public function add_order_metabox() {
			add_meta_box(
				'yith_wcmc_user_preferences',
				__( 'Mailchimp status', 'yith-woocommerce-mailchimp' ),
				array(
					$this,
		...
		public function print_user_preferences_metabox( $post ) {
			...
			$customer_subscribed = $order->get_meta( '_yith_wcmc_customer_subscribed' );
			$personal_data       = $order->get_meta( '_yith_wcmc_personal_data' );
		...
		public function action_links( $links ) {
			$links = yith_add_action_links( $links, 'yith_wcmc_panel', defined( 'YITH_WCMC_PREMIUM' ), YITH_WCMC_SLUG );
		 ...
		 *
		 * @param string $old_value Old key value.
 ...
 * @since 1.0.0
 */
function YITH_WCMC_Admin() { // phpcs:ignore WordPress.NamingConventions.ValidFunctionName.FunctionNameInvalid
