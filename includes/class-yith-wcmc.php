...
if ( ! class_exists( 'YITH_WCMC' ) ) {
	...
	class YITH_WCMC {
				...
				if ( $option['set'] && ! empty( $option_value ) ) {
					foreach ( $option_value as $id => $subset ) {
						if ( ! empty( $option['index'] ) && isset( $subset[ $option['index'] ] ) ) {
							...
							if ( $list ) {
								$option_value[ $id ][ $option['index'] ] = $this->update_groups_to_200_format( $subset[ $option['index'] ], $list );
							}
						} elseif ( empty( $option['index'] ) && ! empty( $option['list_option'] ) ) {
							$option_value[ $id ] = $this->update_groups_to_200_format( $subset, $list );
						}
					}
				} else {
					if ( ! empty( $option['index'] ) && isset( $option_value[ $option['index'] ] ) ) {
						...
						if ( $list ) {
							$option_value[ $option['index'] ] = $this->update_groups_to_200_format( $option_value[ $option['index'] ], $list );
						}
					} elseif ( empty( $option['index'] ) && ! empty( $option['list_option'] ) ) {
						$option_value = $this->update_groups_to_200_format( $option_value, $list );
					}
				}
		 ...
		 *
		 * @param array  $group_options Group array formatted in old style groupID-interestName.
		 * @param string $list          Related list.
		 *
		 * @return array Array of groups formatted in new style groupID-interestID
		 * @since 2.0.0
		 */
		protected function update_groups_to_200_format( $group_options, $list ) {
			...
			if ( ! empty( $group_options ) && ! empty( $list ) && $api_key ) {
				...
				foreach ( $group_options as $option ) {
					...
					foreach ( $list_groups as $new_option => $name ) {
						if ( "{$group_name} - {$interest_name}" === $name ) {
		...
		public function log( $message, $level = 'info' ) {
			...
			$this->log->add( 'yith_wcmc', $message, $level );
		}
		 ...
		 *
		 * @param bool $force_update Whether to force update of cache.
		 *
		 ...
		 *
		 * @param string $list            Id of the list, used to retrieve groups.
		 * @param string $subscriber_hash Hash of the subscriber to Mailchimp.
		 * @param bool   $force_update    Whether to force update of cache.
		 *
		 ...
		 * @param array  $args  Array of additional args to use for the API call.
		 *
		 ...
		 * @param array  $args         Associative array of params to use in the request (default to empty array).
		 * @param bool   $force_update Whether or not to update cached data with a fresh request (applied only for requests in $cachable_requests, default to false).
		 *
		...
		public function do_request( $method, $request = '', $args = array(), $force_update = false ) {
			...
			if ( ! in_array( $method, self::$available_api_methods, true ) ) {
				return false;
			}

			if ( yith_wcmc_doing_batch() && 'get' !== $method && 'batches' !== $request ) {
			...
			$transient_key  = $request . '_' . md5( wp_json_encode( $args ) );
			...
			try {
				...
				// execute API call.
				$result = $this->call_rest_api( $method, $request, $args );
		 ...
		 *
		 * @param string $request Request to delete.
		 *
		 * @return void
		 *
		 * @since 2.0.0
		 ...
		 *
		 * @param bool $empty_list Whether to empty list after read.
		 *
		 ...
		 * @param string $request   Path to call.
		 * @param array  $args      Array of additional args to send with API call.
		 * @param bool   $iteration Reserved; set to true when iterating to get all page of the set.
		 *
		 ...
		 */
		protected function call_rest_api( $method, $request, $args, $iteration = false ) {
			...
			} elseif ( isset( $result['status'] ) && is_int( $result['status'] ) && '200' !== $result['status'] ) {
				throw new YITH_WCMC_API_Exception( $result['detail'], $result['title'], $result['status'] );
			}

			// check if iteration is required.
			if ( 'get' === $method && ( ! $manual_pagination || $iteration ) && isset( $result['total_items'] ) && $result['total_items'] > ( $offset + $count ) ) {
				$args['count']  = $count;
				$args['offset'] = $offset + $count;

				$next_page = $this->call_rest_api( $method, $request, $args, true );
			...
			if ( is_array( $result ) && ( ! isset( $result['status'] ) || '200' === $result['status'] ) ) {
				$result['status'] = true;
			}
		...
		public function do_request_via_ajax() {
			...
			$method       = isset( $_REQUEST['method'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['method'] ) ) : false;
			$request      = isset( $_REQUEST['request'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['request'] ) ) : false;
			$args         = isset( $_REQUEST['args'] ) ? wc_clean( $_REQUEST['args'] ) : array(); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput, WordPress.Security.NonceVerification.Recommended
			$force_update = isset( $_REQUEST['force_update'] ) ? (bool) $_REQUEST['force_update'] : false;
			...
			if ( ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_REQUEST['yith_wcmc_ajax_request_nonce'] ) ), 'yith_wcmc_ajax_request' ) ) {
				wp_send_json( false );
			}
		...
		public function retrieve_groups_via_ajax() {
			...
			// retrieve params for the request.
			$list         = isset( $_REQUEST['list'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['list'] ) ) : false;
			$force_update = isset( $_REQUEST['force_update'] ) ? (bool) $_REQUEST['force_update'] : false;
		...
		public function add_subscription_checkbox() {
			...
			if ( 'never' !== $trigger && $show_checkbox ) {
				$positions_hook_relation = apply_filters(
					'yith_wcmc_checkbox_position_hook',
					array(
				...
				if ( ! in_array( $checkbox_position, array_keys( $positions_hook_relation ), true ) ) {
		...
		public function adds_order_meta( $order_id ) {
			// nonce processing not required, as already procced by WooCommerce during checkout.
			$show_checkbox   = 'yes' === get_option( 'yith_wcmc_subscription_checkbox' );
			$submitted_value = isset( $_POST['yith_wcmc_subscribe_me'] ) ? 'yes' : 'no'; // phpcs:ignore WordPress.Security.NonceVerification.Missing
			$order           = wc_get_order( $order_id );

			$order->update_meta_data( '_yith_wcmc_show_checkbox', $show_checkbox );
			$order->update_meta_data( '_yith_wcmc_submitted_value', $submitted_value );
			$order->save();
		}
		...
		public function subscribe_on_checkout( $order_id ) {
			...
			$show_checkbox   = $order->get_meta( '_yith_wcmc_show_checkbox' );
			$submitted_value = $order->get_meta( '_yith_wcmc_submitted_value' );
		...
		public function subscribe_on_completed( $order_id ) {
			...
			// return if subscription checkbox is printed, but not submitted.
			if ( apply_filters( 'yith_wcmc_subscribe_on_completed', $show_checkbox && 'no' === $submitted_value, $order_id ) ) {
				return false;
			}
		...
		public function order_subscribe( $order_id, $args = array() ) {
			...
			$merge_fields->FNAME = $first_name; // phpcs:ignore WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase
			$merge_fields->LNAME = $last_name; // phpcs:ignore WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase

			$args = array_merge(
				array(
					'email_address'   => $email,
					'merge_fields'    => apply_filters( 'yith_wcmc_subscribe_merge_vars', $merge_fields, $order->get_customer_id() ),
			...
			if ( isset( $res['status'] ) && ! $res['status'] ) {
				...
				$order->add_order_note( sprintf( __( 'MAILCHIMP ERROR: (%1$s) %2$s', 'yith-woocommerce-mailchimp' ), $res['code'], $res['message'] ) );

				return $res;
			}

			do_action( 'yith_wcmc_after_user_subscribing', $order_id, $list_id );

			// register subscribed list.
			$this->register_customer_subscribed_lists( isset( $args['id'] ) ? $args['id'] : $list_id, $email, $user_id, $order_id );

			// register personal data.
			$personal_data = apply_filters(
				'yith_wcmc_customer_personal_data',
				array(
			...
			if ( ! empty( $personal_data ) ) {
				$this->register_customer_personal_data( $order_id, $personal_data );
			}

			$order->update_meta_data( '_yith_wcmc_customer_subscribed', true );
			$order->save();
		 ...
		 * @return void
		 */
		protected function register_customer_personal_data( $order_id, $arg1, $arg2 = false, $arg3 = false ) {
			$order = wc_get_order( $order_id );
			...
			$previous_personal_data = $order->get_meta( '_yith_wcmc_personal_data' );
			...
			$order->update_meta_data( '_yith_wcmc_personal_data', $new_personal_data );
			$order->save();
		}
		 ...
		 * @return void
		 */
		protected function register_customer_subscribed_lists( $list_id, $email, $user_id, $order_id = false ) {
			...
			if ( $user_id ) {
				...
				if ( ! in_array( $email, $order_subscribed_lists[ $list_id ], true ) ) {
			...
			if ( $order_id ) {
				...
				if ( $order ) {
					$order_subscribed_lists = $order->get_meta( '_yith_wcmc_subscribed_lists' );
					$order_subscribed_lists = ! empty( $order_subscribed_lists ) ? $order_subscribed_lists : array();
					...
					if ( ! in_array( $email, $order_subscribed_lists[ $list_id ], true ) ) {
						$order_subscribed_lists[ $list_id ][] = $email;
						$order->update_meta_data( '_yith_wcmc_subscribed_lists', $order_subscribed_lists );
						$order->save();
					}
				}
			}
 ...
 * @since 1.0.0
 */
function YITH_WCMC() { // phpcs:ignore WordPress.NamingConventions.ValidFunctionName.FunctionNameInvalid
