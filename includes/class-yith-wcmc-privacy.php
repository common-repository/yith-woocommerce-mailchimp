...
if ( ! class_exists( 'YITH_WCMC_Privacy' ) ) {
	...
	class YITH_WCMC_Privacy extends YITH_Privacy_Plugin_Abstract {
		...
		public function __construct() {
			...
			add_filter( 'woocommerce_privacy_export_order_personal_data_props', array( $this, 'register_props_to_export_within_order' ), 10, 2 );
			add_filter( 'woocommerce_privacy_export_order_personal_data_prop', array( $this, 'retrieve_prop_to_export_within_order' ), 10, 3 );

			// hook to order anonymizer, to remove personal data sent to MailChimp and unsubscribe user.
			add_action( 'woocommerce_privacy_before_remove_order_personal_data', array( $this, 'remove_props_from_order' ), 10, 1 );
		 ...
		 *
		 * @param string $section Section of the policy to retrieve.
		 *
		 ...
		 *
		 * @param array    $props Array of props to export.
		 * @param WC_Order $order Current order being exported.
		 *
		 ...
		 *
		 * @param string   $value Calculated value for current prop.
		...
		public function remove_props_from_order( $order ) {
			$personal_data    = $order->get_meta( '_yith_wcmc_personal_data' );
			$subscribed_lists = $order->get_meta( '_yith_wcmc_subscribed_lists' );

			$props_to_remove = apply_filters(
				'yith_wcmc_privacy_remove_personal_data_props',
				array(
			...
			if ( ! empty( $personal_data ) ) {
				...
				$order->update_meta_data( '_yith_wcmc_personal_data', $personal_data );
			}
			...
			if ( ! empty( $subscribed_lists ) && apply_filters( 'yith_wcmc_remove_personal_data_unsubscribe_user', false ) ) {
				...
				$order->update_meta_data( '_yith_wcmc_subscribed_lists', array() );
			}
